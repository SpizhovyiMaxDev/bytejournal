...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $upload_path = $images_dir . $avatar_filename;

    if (!is_dir($images_dir)) {
        if (!mkdir($images_dir, 0755, true)) {
            $_SESSION['add-user-error'] = "Failed to create images directory. Please check folder permissions.";
            $_SESSION['add-user-data'] = $_POST;
            header('Location: ' . ROOT_URL . 'admin/add_user.php');
            exit;
        }
    ...
    if (!is_writable($images_dir)) {
        $_SESSION['add-user-error'] = "Images directory is not writable. Please check folder permissions.";
        $_SESSION['add-user-data'] = $_POST;
        header('Location: ' . ROOT_URL . 'admin/add_user.php');
        exit;
    }
    ...
    if (!move_uploaded_file($avatar['tmp_name'], $upload_path)) {
        $_SESSION['add-user-error'] = "Failed to upload avatar. Please try again.";
        $_SESSION['add-user-data'] = $_POST;
        header('Location: ' . ROOT_URL . 'admin/add_user.php');
        exit;
    }
